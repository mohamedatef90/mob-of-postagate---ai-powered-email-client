    ...
    return ReactDOM.createPortal(
        <div className="fixed inset-0 bg-black/30 z-[60] flex items-center justify-center p-4" onClick={onClose}>
            <div className="bg-surface rounded-lg shadow-xl w-full max-w-md" onClick={e => e.stopPropagation()}>
                <h2 className="p-4 font-semibold border-b border-border">Edit Signature</h2>
                <div className="p-4">
                    <textarea value={sig} onChange={e => setSig(e.target.value)} className="w-full h-32 p-2 border rounded bg-background text-foreground" />
                </div>
