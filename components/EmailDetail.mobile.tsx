...
const EmailMessage: React.FC<{ message: Message; allParticipants: User[]; subject: string; isStarred: boolean; onReply: () => void; onToggleStar: () => void; }> = ({ message, allParticipants, subject, isStarred, onReply, onToggleStar }) => {
        ...
        <Card className="p-4 mb-4 animate-fadeInUp">
            <div className="flex justify-between items-start mb-4">
                ...
                <div className="flex items-center space-x-3 text-muted-foreground text-sm flex-shrink-0 ml-2">
                    ...
                    <button className="h-7 w-7 flex items-center justify-center rounded-md hover:bg-accent" title="More options"><i className="fa-solid fa-ellipsis-h w-4 h-4"></i></button>
                </div>
            </div>
            <div 
...
const EmailDetailMobile: React.FC<EmailDetailProps> = ({ thread, summaryState, onClearSummary, onComposeInteraction, onToggleStar }) => {
  if (!thread) {
    return (
  ...
  return (
    <div className="flex flex-col h-full flex-1 overflow-auto">
      <div className="flex-1 overflow-y-auto p-4">
        <h2 className="text-2xl font-bold text-foreground truncate mb-6">{thread.subject}</h2>
