...
const NewChatMobile: React.FC<NewChatMobileProps> = ({ onBack, users, onSelectUser, onCreateGroup }) => {
  ...
  const [selectedDomain, setSelectedDomain] = useState<'hogwarts' | 'liverpool'>('hogwarts');

  const domainUsers = users.filter(user => 
    user.email !== you.email && 
    (selectedDomain === 'hogwarts' 
      ? user.email.endsWith('@hogwarts.uk.edu') 
    ...
    <div className="flex flex-col h-full">
      ...
      <div className="p-2 border-b border-border">
          <div className="flex items-center bg-secondary rounded-lg p-1">
              <button onClick={() => setSelectedDomain('hogwarts')} className={`w-1/2 rounded-md py-1 text-sm font-semibold transition-colors ${selectedDomain === 'hogwarts' ? 'bg-card shadow-sm' : 'hover:bg-card/50'}`}>Hogwarts</button>
